...
function UI.list(arg)
    ...
    view.onSelect = arg.onSelect
    view.items = arg.items
    view.rowHeight = arg.rowHeight or 1
    view.bgAlternate = arg.bgAlternate or arg.bg or colors.lightGray
    view.bgSelected = arg.bgSelected or colors.black
    view.fgSelected = arg.fgSelected or colors.white
    
    view.scrollIndex = 1 -- first item displayed
    view.selected = nil
    ...
    view.onMouseDown = function(self, x, y, button)
        if button ~= 1 then
            return
        end
        if x == self.w - 1 then
        ...
        elseif self.onSelect then
            ...
            if self.onSelect then
                self.onSelect(self, self.selected, self.items[self.selected])
            end
        end
    end
    view.scrollDown = function()
        view.redraw()
        view.scrollIndex = math.min(view.scrollIndex + 1, view.maxScroll)
    end
    view.scrollUp = function()
        view.redraw()
        view.scrollIndex = math.max(1,view.scrollIndex - 1)
    end
    view.onScroll = function(self, direction)
        if direction == -1 then
            view.scrollUp()
